          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', backgroundColor: 'rgba(0,0,0,0.04)', padding: '3px', borderRadius: '10px', gap: '2px' }}>
              {[{ id: 'co2', label: 'CO₂' }, { id: 'noise', label: '소음' }, { id: 'temphumi', label: '온도·습도' }, { id: 'dust', label: '미세먼지' }].map(tab => (
                <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: '6px 14px', borderRadius: '8px', border: 'none', cursor: 'pointer', fontSize: '12px', fontWeight: '700', backgroundColor: activeTab === tab.id ? '#FFFFFF' : 'transparent', color: activeTab === tab.id ? '#1E293B' : '#64748B', boxShadow: activeTab === tab.id ? '0 2px 6px rgba(0,0,0,0.06)' : 'none', transition: 'all 0.2s' }}>{tab.label}</button>
              ))}
            </div>
          </div>
          {activeTab === 'dust' && (
