...
const ParticleBg = ({ color }) => {
  ...
  useEffect(() => {
    ...
    const NUM = 50;
    const particles = Array.from({ length: NUM }, () => ({ x: Math.random() * canvas.width, y: Math.random() * canvas.height, r: Math.random() * 3 + 1, dx: (Math.random() - 0.5) * 0.5, dy: (Math.random() - 0.5) * 0.5, alpha: Math.random() * 0.5 + 0.1 }));
    const draw = () => {
...
const METRIC_CONFIG = [
  { id: 'score', label: '학습 지수', color: '#10B981', axis: 'y' }, { id: 'humi', label: '습도', color: '#3B82F6', axis: 'y' },
  { id: 'co2', label: 'CO₂', color: '#F59E0B', axis: 'y1' }, { id: 'temp', label: '온도', color: '#EF4444', axis: 'y2' },
  { id: 'noise', label: '소음', color: '#8B5CF6', axis: 'y2' }, { id: 'pm10', label: '미세먼지', color: '#64748B', axis: 'y2' }
];
...
const AnalyticsPage = () => {
  ...
  const getStatusLevel = (score) => {
    if (score >= 90) return "매우 쾌적"; if (score >= 75) return "쾌적";
    if (score >= 60) return "보통"; if (score >= 40) return "나쁨"; return "매우 나쁨";
  };
  ...
  useEffect(() => {
    ...
    window.addEventListener('wheel', handleWheel, { passive: false }); window.addEventListener('keydown', handleKeyDown);
      ...
      try {
        ...
        if (res.data && res.data.success && res.data.data) {
          ...
          if (chartData.length > 0) {
            ...
            setSensorData({ score: score, statusText: getStatusLevel(score) });
          }
          ...
          const metrics = {
            score: chartData.map(item => item.spaceScore || 0), co2: chartData.map(item => item.co2 || 0),
            temp: chartData.map(item => item.temperature || 0), humi: chartData.map(item => item.humidity || 0),
            noise: chartData.map(item => item.noise || 0), pm10: chartData.map(item => item.dustPm10 || 0)
          };
          setApiData({ labels: labels, metrics: metrics, summary: rawData.summary || {}, insights: rawData.insights || [] });
        } else {
          setIsDataEmpty(true);
        }
      } catch (err) {
        setIsDataEmpty(true); setApiData(null);
      }
    };
    fetchData();
  }, [timeFilter]);
  ...
  const { bestFocusInsight, co2WarningInsight, goodRatioInsight, avgScoreInsight } = useMemo(() => {
    ...
    return {
      bestFocusInsight: insightsList.find(i => i.type === 'best_focus_time'), co2WarningInsight: insightsList.find(i => i.type === 'co2_warning'),
      goodRatioInsight: insightsList.find(i => i.type === 'good_ratio'), avgScoreInsight: insightsList.find(i => i.type === 'average_score')
    };
  }, [apiData]);

  const generateLabels = (filter) => {
    const generatedLabels = []; const now = new Date(); let hours = 6; let points = 24; 
    if (filter === '1시간') hours = 1; else if (filter === '6시간') hours = 6; else if (filter === '12시간') hours = 12; else if (filter === '24시간') hours = 24;
  ...
  const datasets = METRIC_CONFIG.filter(metric => activeMetrics.includes(metric.id)).map(metric => ({
    id: metric.id, label: metric.label, data: apiData?.metrics?.[metric.id] || [], 
    borderColor: metric.color, backgroundColor: 'transparent', yAxisID: metric.axis, tension: 0.3, borderWidth: 2.5, pointRadius: 0, pointHoverRadius: 6,
  }));

  const chartData = { labels, datasets };
  const chartOptions = {
    responsive: true, maintainAspectRatio: false, layout: { padding: { top: 20, right: 20, bottom: 0, left: 10 } },
    scales: {
      y: { type: 'linear', position: 'left', suggestedMin: 0, suggestedMax: 100, display: activeMetrics.some(m => ['score', 'humi'].includes(m)), ticks: { color: '#94A3B8', font: { size: 10 } }, grid: { color: 'rgba(0,0,0,0.04)' } },
      y1: { type: 'linear', position: 'right', suggestedMin: 400, suggestedMax: 1500, display: activeMetrics.includes('co2'), ticks: { color: '#F59E0B', font: { size: 10 } }, grid: { drawOnChartArea: false } },
      y2: { type: 'linear', position: 'right', suggestedMin: 0, suggestedMax: 60, display: activeMetrics.some(m => ['temp', 'noise', 'pm10'].includes(m)), ticks: { color: '#64748B', font: { size: 10 } }, grid: { drawOnChartArea: false } },
  ...
  const summaryKeyMap = { co2: 'co2', temp: 'temperature', humi: 'humidity', noise: 'noise', pm10: 'dustPm10' };
          ...
          {[{ label: '홈', sub: '현재 상태', path: '/' }, { label: '대시보드', sub: '실시간 센서', path: '/dashboard' }, { label: '통계', sub: '기록 분석', path: '/analytics' }, { label: '설정', sub: '환경 설정', path: '/settings' }].map(({ label, sub, path }) => {
            ...
            return (
              <div key={label} onClick={() => navigate(path)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '12px 14px', borderRadius: '10px', cursor: 'pointer', background: isActive ? `linear-gradient(90deg, ${theme.color}22, transparent)` : 'transparent', borderLeft: isActive ? `3px solid ${theme.color}` : '3px solid transparent', transition: 'all 0.2s ease', textAlign: 'center' }}>
                <div style={{ fontSize: '13px', fontWeight: isActive ? '700' : '500', color: isActive ? '#FFF' : '#6B7A99' }}>{label}</div><div style={{ fontSize: '10px', color: '#4A5568', marginTop: '4px' }}>{sub}</div>
              </div>
            );
          })}
        </nav>
        ...
        {!isConnected ? (
          ...
          <>
            <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', zIndex: 10 }}>
              <div><h1 style={{ fontSize: '28px', fontWeight: '800', margin: 0, color: '#1A202C', letterSpacing: '-0.5px' }}>통계 및 기록</h1><p style={{ fontSize: '14px', color: '#64748B', margin: '8px 0 0 0' }}>과거 기록을 통해 패턴을 분석합니다</p></div>
              <div style={{ ...glassCardStyle, padding: '4px', display: 'flex', borderRadius: '12px' }}>
                {['1시간', '6시간', '12시간', '24시간'].map(time => {
                  ...
                  return (
                    <button key={time} onClick={() => handleFilterClick(time)} style={{ padding: '8px 18px', fontSize: '13px', fontWeight: isActive ? '700' : '600', color: isActive ? theme.color : '#64748B', backgroundColor: isActive ? 'rgba(255,255,255,0.9)' : 'transparent', borderRadius: '8px', cursor: 'pointer', border: 'none', boxShadow: isActive ? '0 2px 8px rgba(0,0,0,0.05)' : 'none', transition: 'all 0.2s' }}>{time}</button>
                  );
                })}
              </div>
            </header>
              ...
              {METRIC_CONFIG.map(metric => {
                ...
                return (
                  <div key={metric.id} onClick={() => toggleMetric(metric.id)} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 16px', borderRadius: '24px', backgroundColor: isActive ? metric.color : 'rgba(255,255,255,0.7)', color: isActive ? '#FFF' : '#4A5568', fontSize: '13px', fontWeight: '700', cursor: 'pointer', transition: 'all 0.2s', border: isActive ? '1px solid transparent' : '1px solid rgba(255,255,255,0.9)', boxShadow: isActive ? `0 4px 12px ${metric.color}40` : '0 2px 8px rgba(0,0,0,0.03)', backdropFilter: 'blur(8px)' }}>
                    {!isActive && <div style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: metric.color }} />}{metric.label}
                  </div>
                );
              })}
            </div>
            ...
            <div style={{ display: 'flex', gap: '24px', flex: 1, minHeight: 0, zIndex: 10 }}>
              <section style={{ ...glassCardStyle, flex: '7', padding: '28px', display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ position: 'relative', flex: 1, width: '100%', minHeight: '400px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {isDataEmpty ? <div style={{ fontSize: '14px', color: '#94A3B8', fontWeight: '600' }}>해당 기간에 데이터가 없습니다.</div> : <Line key={timeFilter + (apiData ? '-data-loaded' : '-loading')} data={chartData} options={chartOptions} />}
                </div>
              </section>
              ...
              <section style={{ flex: '3', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div style={{ ...glassCardStyle, padding: '24px', overflowY: 'auto' }}>
                  ...
                  <div style={{ backgroundColor: 'rgba(255,255,255,0.6)', borderRadius: '12px', padding: '16px', marginBottom: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', fontWeight: '700', color: '#10B981', marginBottom: '6px' }}>✨ {bestFocusInsight?.title || '최고 집중 시간대'}</div>
                    <div style={{ fontSize: '18px', fontWeight: '800', color: bestFocusInsight?.time ? '#10B981' : '#94A3B8', marginBottom: '4px', letterSpacing: '-0.5px' }}>{bestFocusInsight?.time ? new Date(bestFocusInsight.time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : '데이터 없음'}</div>
                    {bestFocusInsight?.message && <div style={{ fontSize: '12px', color: '#64748B' }}>{bestFocusInsight.message}</div>}
                  </div>
                  
                  <div style={{ backgroundColor: 'rgba(255,255,255,0.6)', borderRadius: '12px', padding: '16px', marginBottom: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', fontWeight: '700', color: '#F59E0B', marginBottom: '6px' }}>⚠️ {co2WarningInsight?.title || '최고 CO₂ 시점'}</div>
                    <div style={{ fontSize: '18px', fontWeight: '800', color: co2WarningInsight?.time ? '#F59E0B' : '#94A3B8', marginBottom: '4px', letterSpacing: '-0.5px' }}>{co2WarningInsight?.time ? new Date(co2WarningInsight.time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : '데이터 없음'}</div>
                    {co2WarningInsight?.message && <div style={{ fontSize: '12px', color: '#64748B' }}>{co2WarningInsight.message}</div>}
                  </div>

                  <div style={{ backgroundColor: 'rgba(255,255,255,0.6)', borderRadius: '12px', padding: '16px', marginBottom: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', fontWeight: '700', color: '#6366F1', marginBottom: '6px' }}>✅ {goodRatioInsight?.title || '쾌적 환경 비율'}</div>
                    <div style={{ fontSize: '18px', fontWeight: '800', color: goodRatioInsight ? '#6366F1' : '#94A3B8', marginBottom: '4px', letterSpacing: '-0.5px' }}>{goodRatioInsight ? (goodRatioInsight.value || '0%') : '데이터 없음'}</div>
                    {goodRatioInsight?.message && <div style={{ fontSize: '12px', color: '#64748B' }}>{goodRatioInsight.message}</div>}
                  </div>

                  <div style={{ backgroundColor: 'rgba(255,255,255,0.6)', borderRadius: '12px', padding: '16px', marginBottom: '12px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', fontWeight: '700', color: '#4A5568', marginBottom: '6px' }}>🎯 {avgScoreInsight?.title || '평균 학습 지수'}</div>
                    <div style={{ fontSize: '18px', fontWeight: '800', color: avgScoreInsight ? '#4A5568' : '#94A3B8', marginBottom: '4px', letterSpacing: '-0.5px' }}>{avgScoreInsight ? (avgScoreInsight.value || '0점') : '데이터 없음'}</div>
                    {avgScoreInsight?.message && <div style={{ fontSize: '12px', color: '#64748B' }}>{avgScoreInsight.message}</div>}
                  </div>
                  ...
                  <div style={{ marginTop: '24px', paddingTop: '16px', borderTop: '1px dashed rgba(0,0,0,0.1)' }}>
                    ...
                    {isDataEmpty ? (
                      ...
                      [{ label: 'CO₂', id: 'co2', color: '#F59E0B', unit: 'ppm' }, { label: '온도', id: 'temp', color: '#EF4444', unit: '°C' }, { label: '습도', id: 'humi', color: '#3B82F6', unit: '%' }, { label: '소음', id: 'noise', color: '#8B5CF6', unit: 'dB' }, { label: '미세먼지', id: 'pm10', color: '#64748B', unit: 'µg' }].map((item, idx) => {
                        const backendKey = summaryKeyMap[item.id]; const backendAvg = apiData?.summary?.[backendKey]?.avg ?? '0.0';
                        return (
                          <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '13px', padding: '10px 0', borderBottom: idx === 4 ? 'none' : '1px solid rgba(0,0,0,0.04)' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: '#4A5568', fontWeight: '600' }}><span style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: item.color }} />{item.label}</div>
                            <div style={{ fontWeight: '700', color: '#1A202C', fontFamily: "'DM Mono', monospace" }}>avg {backendAvg} <span style={{ fontSize: '11px', color: '#94A3B8', fontFamily: "'Pretendard', sans-serif" }}>{item.unit}</span></div>
                          </div>
                        );
                      })
                    )}
